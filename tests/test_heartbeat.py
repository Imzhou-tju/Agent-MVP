...
class TestHeartbeatPacemaker(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 保存原始 TASKS_FILE 路径
        import clawgent.core.config
        self.original_tasks_file = clawgent.core.config.TASKS_FILE
        
        # 设置临时任务文件
        clawgent.core.config.TASKS_FILE = self.temp_file.name
        
        # 同时 patch heartbeat 模块中的引用
        import clawgent.core.heartbeat
        clawgent.core.heartbeat.TASKS_FILE = self.temp_file.name
    ...
    def tearDown(self):
        ...
        # 恢复原始路径
        import clawgent.core.config
        clawgent.core.config.TASKS_FILE = self.original_tasks_file
        
        import clawgent.core.heartbeat
        clawgent.core.heartbeat.TASKS_FILE = self.original_tasks_file

    def test_no_tasks_file(self):
        """测试任务文件不存在时的行为"""
        from clawgent.core.heartbeat import pacemaker_loop
        ...
        async def run_test():
            # 直接测试逻辑，不实际等待
            import clawgent.core.heartbeat as hb
            # 模拟 TASKS_FILE 不存在
